...
function setStartPoint(){
  ...
  updateStatus("GPS Acquired, Start Running");
  setInterval(() => makeLap(),2000);
//   timer.start();
  return true;
  } else {
...
function makeLap(){
  distanceCovered += (calculateDistance(startPosition[0],startPosition[1],currentPosition[0],currentPosition[1])* 1000);
  startPosition = [currentPosition[0],currentPosition[1]]
}
...
function updateGPSData(){
  ...
  try{
  select("#distance-covered").html("Distance: "+(calculateDistance(startPosition[0],startPosition[1],currentPosition[0],currentPosition[1])+distanceCovered).toFixed(4)+"M")
  }catch{}
}
